...
describe('components/Icon', () => {
  ...
  describe('Render icon without passing props', () => {
    ...
    describe('Render icon with passing props', () => {
      it.each(Object.values(ComponentSizeEnum))('should render with size = "%s"', (size) => {
        setup({ size });

        expect(iconRef.current).toHaveAttribute('width', `${SizeMapping[size]}px`);
      });

      it.each(Object.values(IconNameEnum))('should render with name = "%s" and without pathElement prop', (name) => {
        setup({ name });
        expect(pathElement).toHaveAttribute('d', icons[name]);
      });

      it.each(Object.values(IconNameEnum))('should render with name = "%s" and with pathElement prop', (name) => {
        const path = 'testPath';
        setup({ name, path });
