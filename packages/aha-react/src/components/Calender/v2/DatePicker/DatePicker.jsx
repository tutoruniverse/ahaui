...
export default class DatePicker extends PureComponent {
  ...
  onOutsideAction = (event) => {
    const { hideOnUnfocus } = this.props;
    if (hideOnUnfocus && this.wrapper && !this.wrapper.contains(event.target)) {
      this.closeCalendar();
    }
  }

  onChange = (value, closeCalendar = true, isValid = true) => {
    const { closeOnChange } = this.props;
    this.setState({
      isOpen: !(closeOnChange && closeCalendar),
      isValid,
    });

    const { onChange, onValidating } = this.props;
    if (onChange) {
      onChange(value);
    }
    if (onValidating) {
