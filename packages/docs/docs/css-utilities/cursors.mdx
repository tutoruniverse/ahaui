  ...
  <tbody>
    {classExtractCursors.map((item) => (
      <tr key={item.index}>
        ...
        <td className="u-textGray">{item.description}</td>
      </tr>
    ))}
  </tbody>
</table>
